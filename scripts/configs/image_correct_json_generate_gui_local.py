...
def generate_config(setting_dict):
    ...
    if config_dict["topo"]['type']=='precomputed':
        ...
        if setting_dict["input_datasets"]["n_subgroups"]>1:
            config_dict["topo"]["subgrouped"] = True
            config_dict["topo"]["sample_perc"] = 0.2
            config_dict["topo"]["subgroup"] = {}
            #print(setting_dict["input_datasets"]["group_images"])
            config_dict["topo"]["subgroup"] = setting_dict["input_datasets"]["group_images"]
...
def process_and_reorder_config(raw_data):
    ...
    structured_config_from_gui = {
        ...
        "input_datasets": {
            ...
            "n_subgroups":inputs.get('subgroups'),
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Local Server Job Configurator for Image Correction by HyTools")
        ...
        self.group_name_vars = ['Group 1', 'Group 2']  #[]
        self.file_group_vars = {} # Maps filepath to a StringVar (holding the selected group name)
        self.parsed_ascii_data = None

        self.build_ui()
        #self.update_grouping_ui()
        self.update_suffix_entry()
    ...
    def build_ui(self):
        ...
        ttk.Label(cols_frame, text="TOPO Group Column:").grid(row=0, column=2, sticky="w", padx=4)
        self.ascii_col_topo = ttk.Combobox(cols_frame, state="readonly")
        self.ascii_col_topo.grid(row=1, column=2, sticky="ew", padx=4)
        ...
        # 2. File Grouping
        f2 = ttk.LabelFrame(self.container, text=" 2. Group Selected Reflectance for TOPO correction subgroups (Optional) ", padding=15)
        f2.pack(fill="x", pady=5, padx=5)
        
        ctrl_frame = ttk.Frame(f2)
        ctrl_frame.pack(fill="x", pady=(0, 10))
        ttk.Label(ctrl_frame, text="Number of TOPO subgroups: ").pack(side="left")
        self.num_groups_var = tk.IntVar(value=2)
        
        spin = ttk.Spinbox(ctrl_frame, from_=1, to=20, width=5, textvariable=self.num_groups_var, command=self.on_group_count_changed)
        spin.pack(side="left")
        ...
        # Left: Group Names
        self.group_names_frame = ttk.LabelFrame(split_frame, text="Define Group Names", padding=5)
        self.group_names_frame.pack(side="left", fill="both", expand=True, padx=(0, 5))

        # Right: File Assignments
        self.file_assign_frame = ttk.LabelFrame(split_frame, text="Assign Files to Groups", padding=5)
        self.file_assign_frame.pack(side="right", fill="both", expand=True, padx=(5, 0))        

        # 3. Ancillary Data
        f3 = ttk.LabelFrame(self.container, text=" 3. Ancillary images in ENVI format paired with reflectance images (e.g. *obs_ort, *obs ) \n     Leave it blank for NEON AOP images", padding=15)
        ...
        # 4. Precomputed correction model coefficients 
        f4 = ttk.LabelFrame(self.container, text=" 4. Precomputed correction model coefficients ", padding=15)
        ...
        btn_f1.pack(fill="x")
        self.btn_supp_a = ttk.Button(btn_f1, text="Browse TOPO JSON Files...", command=lambda: self.select_files(self.lb_supp_a, self.supp_a_files, filetypes=[("JSON Files","*.json;*.JSON")]))
        self.btn_supp_a.pack(side="left", pady=(0, 10))
        ttk.Button(btn_f1, text="Clear", command=lambda: self.clear_listbox(self.lb_supp_a, self.supp_a_files)).pack(side="left", pady=(0, 10),padx=2)
        ...
        btn_f2.pack(fill="x")        
        self.btn_supp_b = ttk.Button(btn_f2, text="Browse BRDF JSON Files...", command=lambda: self.select_files(self.lb_supp_b, self.supp_b_files, filetypes=[("JSON Files","*.json;*.JSON")]))
        self.btn_supp_b.pack(side="left", pady=(0, 10))
        ttk.Button(btn_f2, text="Clear", command=lambda: self.clear_listbox(self.lb_supp_b, self.supp_b_files)).pack(side="left", pady=(0, 10),padx=2)
        ...
        # 5. Use GLT
        f8 = ttk.LabelFrame(self.container, text=" 5. Use External GLT for location adjustment ", padding=15)
        ...
        # 6. Parameters
        f5 = ttk.LabelFrame(self.container, text=" 6. Correction Settings ", padding=15)
        ...
        self.var_out_band_subset = tk.StringVar(value="subset1")
        ttk.Label(col1, text="Bands in output reflectance images", font=("", 9, "bold")).pack(anchor="w")
        ttk.Radiobutton(col1, text="Full range", variable=self.var_out_band_subset, value="full").pack(anchor="w")
        ttk.Radiobutton(col1, text="True color ([660,550,440]nm)", variable=self.var_out_band_subset, value="truecolor").pack(anchor="w")
        ttk.Radiobutton(col1, text="Selected bands (around [440,550,660,850,976,1650,2217]nm)", variable=self.var_out_band_subset, value="subset1").pack(anchor="w", pady=(0, 10))



        self.var_gen_image = tk.StringVar(value="image")
        self.var_gen_coeffs = tk.StringVar(value="")
        self.var_gen_masks = tk.StringVar(value="")
        ttk.Label(col2, text="Export options", font=("", 9, "bold")).pack(anchor="w")
        ttk.Checkbutton(col2, text="Export Images", variable=self.var_gen_image, onvalue="image", offvalue="").pack(anchor="w")
        ttk.Checkbutton(col2, text="Correction Model Coefficients", variable=self.var_gen_coeffs, onvalue="coeffs", offvalue="").pack(anchor="w")
        ttk.Checkbutton(col2, text="Save correction related masks", variable=self.var_gen_masks, onvalue="masks", offvalue="").pack(anchor="w", pady=(0, 10))
        ...
        ttk.Checkbutton(col2, text="Topographic Correction (topo)", variable=self.var_topo_bool, onvalue="topo", offvalue="",command=self.update_suffix_entry).pack(anchor="w")
        ttk.Checkbutton(col2, text="BRDF correction (brdf)", variable=self.var_brdf_bool, onvalue="brdf", offvalue="",command=self.update_suffix_entry).pack(anchor="w")        
        ttk.Checkbutton(col2, text="Sunglint correction (glint)", variable=self.var_glint_bool, onvalue="glint", offvalue="",command=self.update_suffix_entry).pack(anchor="w", pady=(0, 10))


        # Predefined list of options
        ttk.Label(col2, text="Select TOPO Method", font=("", 9, "bold")).pack(anchor="w")

        topo_options = [ "cosine","c","scs","scs+c","mod_minneart","precomputed"]

        # Create the Combobox
        self.topo_combo = ttk.Combobox(col2, values=topo_options, state="readonly")
        self.topo_combo.set("Select a TOPO method")  # Set placeholder text
        self.topo_combo.pack(anchor="w", pady=(0, 10))
        self.topo_combo.current(3)

        # Bind the selection event to the trigger function

        # Predefined list of options
        ttk.Label(col2, text="Select BRDF Method", font=("", 9, "bold")).pack(anchor="w")
        brdf_options = [ "universal","flex","precomputed"]

        # Create the Combobox
        self.brdf_combo = ttk.Combobox(col2, values=brdf_options, state="readonly")
        self.brdf_combo.pack(anchor="w")
        self.brdf_combo.current(1)

        self.var_brdf_group_mode = tk.StringVar(value="group")
        ttk.Label(col2, text="BRDF grouping mode", font=("", 9, "bold")).pack(anchor="w")
        ttk.Radiobutton(col2, text="Same BRDF model for all images", variable=self.var_brdf_group_mode, value="group").pack(anchor="w")
        ttk.Radiobutton(col2, text="Individual BRDF model for each image", variable=self.var_brdf_group_mode, value="single").pack(anchor="w")

        self.var_out_solar_zn = tk.StringVar(value="scene")
        ttk.Label(col2, text="Target solar zenith angle mode", font=("", 9, "bold")).pack(anchor="w")
        ttk.Radiobutton(col2, text="Normalized to the averaged angle of all images (scene)", variable=self.var_out_solar_zn, value="scene").pack(anchor="w")
        ttk.Radiobutton(col2, text="Normalized to the averaged angle of each image (line)", variable=self.var_out_solar_zn, value="line").pack(anchor="w", pady=(0, 10))

        ttk.Label(col2, text="Select Sunglint Method", font=("", 9, "bold")).pack(anchor="w")
        glint_options = ["hochberg","hedley","gao"]

        # Create the Combobox
        self.glint_combo = ttk.Combobox(col2, values=glint_options, state="readonly")
        #self.glint_combo.set("Select a Sunglint method")  # Set placeholder text
        self.glint_combo.pack(anchor="w")
        self.glint_combo.current(2)

        self.var_out_glint_ref = tk.IntVar(value=1650)
        ttk.Label(col2, text="Reference wavelength for sunglint correction", font=("", 9, "bold")).pack(anchor="w")
        ttk.Radiobutton(col2, text="860nm (NIR)", variable=self.var_out_glint_ref, value=860).pack(anchor="w")
        ttk.Radiobutton(col2, text="1650nm (SWIR)", variable=self.var_out_glint_ref, value=1650).pack(anchor="w")
        ttk.Radiobutton(col2, text="2190nm (SWIR)", variable=self.var_out_glint_ref, value=2190).pack(anchor="w", pady=(0, 10))
        ...
        ttk.Label(export_grid, text="Suffix for output image:", font=("", 9, "bold")).grid(row=0, column=2, sticky="w", pady=2,padx=5)
        self.suffix_var = tk.StringVar(value="")
        ttk.Entry(export_grid, textvariable=self.suffix_var, width=30).grid(row=1, column=2, sticky="nw",pady=2,padx=5)
    ...
    def select_files(self, listbox, target_list, update_groups=False, filetypes=None):
        ...
        if files:
            ...
            if update_groups:
                ...
                self.render_file_assignment_ui()
    ...
    def load_ascii_file(self):
        ...
        try:
            ...
            for idx, hdr in enumerate(headers):
                ...
                if any(k in lower for k in ['topo', 'group', 'sub']):
    ...
    def apply_ascii_data(self):
        ...
        if ref_idx < 0 or anc_idx < 0 or topo_idx < 0:
            return
        ...
        if unique_subgroups:
            self.group_name_vars = unique_subgroups
            self.num_groups_var.set(len(unique_subgroups))
            self.file_group_vars = {
                file: self.group_name_vars.index(sg) for file, sg in file_to_subgroup.items()
            }
            self.render_grouping_ui()

        self.ascii_status_lbl.config(
            text=f"✓ Auto-loaded {len(self.primary_files)} Reflectance, {len(self.anc_files)} Ancillary files, and {len(unique_subgroups)} TOPO Subgroups."
        )
    ...
    def update_grouping_ui(self, *args):
        ...
        # Redraw Group Names
        #if self.group_names_container.winfo_exists():
            #self.group_names_container.configure(text="Updated Text")
    ...
    def update_suffix_entry(self):
        corr_list = [v for v in [self.var_topo_bool.get(), self.var_brdf_bool.get(), self.var_glint_bool.get()] if v]
        if len(corr_list) < 1:
            self.suffix_var.set("raw")
        else:
            self.suffix_var.set('_'.join(corr_list))

    def update_file_assignments(self):
        for widget in self.file_assign_frame.winfo_children():
            widget.destroy()
 
        if not self.primary_files:
            ttk.Label(self.file_assign_frame, text="No primary files selected.").pack(anchor="w")
            return

        self.comboboxes = []
        for filepath in self.primary_files:
            filename = os.path.basename(filepath)
            row = ttk.Frame(self.file_assign_frame)
            row.pack(fill="x", pady=2)

            ttk.Label(row, text=f"📄 {filename}", width=25, anchor="w").pack(side="left", padx=(0,10))
            
            if filepath not in self.file_group_vars:
                self.file_group_vars[filepath] = self.group_name_vars[0]  #tk.StringVar(value=self.group_name_vars[0].get())  #tk.StringVar(value=self.group_name_vars[0].get())

            cb = ttk.Combobox(row, textvariable=self.file_group_vars[filepath], state="readonly")
            cb.pack(side="left", fill="x", expand=True)
            self.comboboxes.append(cb)

        self.refresh_combobox_options()

    def refresh_combobox_options(self):
        options = [var.get() for var in self.group_name_vars]
        for cb in getattr(self, 'comboboxes', []):
            cb['values'] = options
            if cb.get() not in options and options:
                cb.set(options[0])

    ######
    def on_group_count_changed(self):
        try:
            n = int(self.num_groups_var.get())
        except ValueError:
            return

        if n < 1: return
        while len(self.group_name_vars) < n:
            self.group_name_vars.append(f"Group {len(self.group_name_vars) + 1}")
        if len(self.group_name_vars) > n:
            self.group_name_vars = self.group_name_vars[:n]

        self.render_grouping_ui()


    def render_grouping_ui(self):
        self.render_group_names_ui()
        self.render_file_assignment_ui()

    def render_group_names_ui(self):
        for child in self.group_names_frame.winfo_children():
            child.destroy()

        for idx, name in enumerate(self.group_name_vars):
            row = ttk.Frame(self.group_names_frame)
            row.pack(fill="x", pady=2)
            ...
            e.bind("<FocusOut>", lambda ev, i=idx, v=var: self._update_group_name(i, v.get()))

    def _update_group_name(self, idx, new_val):
        self.group_name_vars[idx] = new_val
        self.render_file_assignment_ui()

    def render_file_assignment_ui(self):
        #for child in self.file_assign_frame.winfo_children():
        for child in self.file_assign_frame.winfo_children():
            child.destroy()

        if not self.primary_files:
            ttk.Label(self.file_assign_frame, text="No primary files loaded.", foreground="gray").pack()
            return
        ...
        for filepath in self.primary_files:
            ...
            ttk.Label(row, text=f"📄 {filename}", width=25, anchor="w").pack(side="left", padx=2)

            curr_idx = self.file_group_vars.get(filepath, 0)
            if curr_idx >= len(self.group_name_vars):
                curr_idx = 0
                self.file_group_vars[filepath] = 0

            cb = ttk.Combobox(row, values=self.group_name_vars, state="readonly")
            cb.current(curr_idx)
            cb.pack(side="left", fill="x", expand=True)
            cb.bind("<<ComboboxSelected>>", lambda ev, fp=filepath, widget=cb: self._set_file_assignment(fp, widget.current()))

    def _set_file_assignment(self, filepath, group_idx):
        self.file_group_vars[filepath] = group_idx

    ######

    #def on_topo_select(self,event):
    #    # Retrieve the selected item directly from the combobox
    #    selected_item = self.topo_combo.get()
    #    self.topo_label.config(text=f"Selected: {selected_item}")

    def clear_primary_files(self):
        self.primary_files.clear()
        self.lb_primary.delete(0, tk.END)
        #self.b1_listbox.delete(0, tk.END)
        self.update_file_assignments()  #render_file_assignment_ui()
    ...
    def collect_raw_payload(self):
        ...
        grouped_files = {fpath: self.group_name_vars[self.file_group_vars[fpath]] for fpath in self.file_group_vars}

        tasks = [v for v in [self.var_gen_image.get(), self.var_gen_coeffs.get(), self.var_gen_masks.get()] if v]
        preprocs = [v for v in [self.var_topo_bool.get(), self.var_brdf_bool.get(), self.var_glint_bool.get()] if v]
        ...
        return {
            ...
            "inputs": {
                ...
                "primary_files_grouped": grouped_files,
                "subgroups":len(self.group_name_vars),
                "anc_files": self.anc_files,
                "enable_precomputed_correction": self.supp_enabled_var.get(),
                "topo_json_files": self.supp_a_files if self.supp_enabled_var.get() else [], #topo
                "brdf_json_files": self.supp_b_files if self.supp_enabled_var.get() else [], #brdf
            ...
            "parameters": {
                ...
                "subset_waves":self.var_out_band_subset.get(),
                "bad_bands":self.var_in_band_subset.get(),
                "tasks": tasks,
                "corrections": preprocs,
                "suffix": self.suffix_var.get().strip(),
                "topo_method":self.topo_combo.get(),
                "brdf_method":self.brdf_combo.get(),
                "brdf_group_mode": self.var_brdf_group_mode.get(),
                "brdf_solar_zn":self.var_out_solar_zn.get(),
                "glint_method":self.glint_combo.get(),
                "glint_ref_wave":self.var_out_glint_ref.get(),
            }
        }
