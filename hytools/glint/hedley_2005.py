...
def apply_hedley_2005_correction(hy_obj,data,dimension,index):
    ...
    if dimension == 'line':
        ...
        correction = hy_obj.ancillary['hedley_nir_swir_diff'] * slopes
...
def optimize_slopes(hy_obj):
    deep_water = hy_obj.get_chunk(
        *hy_obj.glint['deep_water_sample']
    )

    deep_correction = deep_water[:,:,hy_obj['correction_band']].flatten()
...
def nir_swir_diff(hy_obj):
    nir_swir_array = hy_obj.get_wave(hy_obj.glint['correction_wave'])
