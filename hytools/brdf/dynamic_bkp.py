# -*- coding: utf-8 -*-
"""
This module contains functions apply an empiral BRDF correction describedthe following paper:
...
def dynamic_mask(hy_obj):
    ...
    brdf_mask = np.zeros((hy_obj.lines, hy_obj.columns))

    for bin_num in hy_obj.brdf['bins']:
        start,end =  hy_obj.brdf['bins'][bin_num]
        brdf_mask[(ndvi > start) & (ndvi <= end)] = bin_num

    #Exclude from analysis
    brdf_mask[hy_obj.get_anc('sensor_zn') < hy_obj.brdf['sensor_zn_min']] = 0
    brdf_mask[ hy_obj.get_band(0) == hy_obj.no_data] = 0
    brdf_mask[~k_finite] = 0
    brdf_mask = brdf_mask.astype(np.int8)
    hy_obj.ancillary['ndvi_classes'] = brdf_mask
...
def ndvi_bins(ndvi,brdf_dict):
   ...
   ndvi_thres = [brdf_dict['ndvi_min']]
   ndvi_thres += ndvi_break_dyn_bin.tolist() 
   ndvi_thres += [brdf_dict['ndvi_max']]
...
def get_solar_zn(hy_obj):
    ...
    ir = hy_obj.get_wave(850)
    return solar_zn[ir != hy_obj.no_data]
...
def calc_dynamic_group(actors,brdf_dict):
    ...
    # Calculate bins and create NDVI class masks for each image
    bins = ndvi_bins(ndvi,brdf_dict)
    bins = dict([(i,b) for i,b in enumerate(bins,start=1)])
    brdf_dict['bins'] = bins
    ...
    _ = ray.get([a.do.remote(dynamic_mask) for a in actors])
    ...
    brdf_dict['mean_solar_zenith'] = float(np.concatenate(solar_zn_samples).mean())
...
def apply_dynamic(hy_obj,data,dimension,index):
    ...
    # if hy_obj.brdf['grouped'] == True:
    #     solar_zn = hy_obj.brdf['mean_solar_zenith']
    # else:
    #     solar_zn = hy_obj.get_anc('solar_zn')
    ...
    if 'interpolators' not in hy_obj.brdf:
        ...
        for index in hy_obj.brdf['coeffs']:
            coeffs= np.array(hy_obj.brdf['coeffs'][index])
            interpolator = interp1d(bin_centers, coeffs, kind = hy_obj.brdf['interp_kind'],
                                    axis=0,fill_value="extrapolate")
            hy_obj.ancillary['interpolators'][index] = interpolator
    ...
    if dimension == 'line':
        ...
        correction_factor[:,hy_obj.ancillary['ndvi'][index] == 0] = 1
        ...
        data[:,brdf_bands] = data[:,brdf_bands]*np.moveaxis(correction_factor,0,-1)
        ...
        correction_factor = brdf_nadir/brdf
        correction_factor[hy_obj.ancillary['ndvi'] == 0] = 1
        ...
        correction_factor = brdf_nadir/brdf
        correction_factor[hy_obj.ancillary['ndvi'][y1:y2,x1:x2] == 0] = 1
        data[:,:,brdf_bands] = data[:,:,brdf_bands]*correction_factor
        ...
        correction_factor = brdf_nadir/brdf
        correction_factor[hy_obj.ancillary['ndvi'][y,x] == 0] = 1
        data[:,brdf_bands] = data[:,brdf_bands]*correction_factor
