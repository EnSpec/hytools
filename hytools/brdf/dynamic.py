...
def class_brdf(actors,brdf_dict):
    ...
    if brdf_dict['grouped']:
        actors = calc_class_group(actors,brdf_dict)
    else:
        _ = ray.get([a.do.remote(calc_class_single,brdf_dict) for a in actors])
...
def calc_class_single(hy_obj,brdf_dict):
...
def calc_class_group(actors,brdf_dict):
...
def apply_class(hy_obj,data,dimension,index):
